...
The Arduino-All-Sensors-v2 was used to test each sensor simultaneously on the Arduino. 
Arduino-Thingspeak-ESP8266-SHEILD-2 is the final source code for this project. 

Our group had decided to use a ESP8266 Wifi chip to send the data to a server (https://thingspeak.com/channels/95915/).

Most of the code is referenced back to the Arduino-All-Sensors-v2. I do not take credit for setting up the server. Another member had done most of the research on how to set the server up. 
For the final source code, I had done tests, evaluations, and documenting to see if there were any calculation errors for each of the sensors or if any of the lines were incorrect or unnecessary.
I also did checks on hardware and the 3D printing for the casing of the project. 

We had also scrapped the moisture sensor because of hardware and software issues.
*/
...
#include <SFE_BMP180.h> //Barometer/Pressure
#include "DHT.h" //Measure temperature

//Server that will recieve all sensors data
//-- IoT Information
#define SSID "xxx"
...
void setup() 
{
  ...
  if(esp.find("OK"))
  {
    ...
    if(!connectWiFi())
    {
      Serial.println(F("ESP8266 Wifi Connect Error!"));
      goto reconnect; //go to label "reconnect"
    }
  }
  else
  {
    Serial.println(F("ESP8266 Serial Connection Error!"));
    Serial.println(F("Soft restting!"));
  ...
  Serial.println(F("TSL2561 Powerup..."));
...
{
  Serial.println("");
  float humidity = DHT22Read(0);// turn integer to string
  double temp1 = DHT22Read(2);// turn integer to string
  double temp2 = BMP180Read(0);// turn integer to string
  double pressure = BMP180Read(1);// turn integer to string 
  int light = TSL2561Read();// turn integer to string
  int soil = analogRead(A0);// turn integer to string
  Serial.print("Soil Moisture: "); Serial.println(soil);
  updateTS(String(humidity),String(temp1),String(pressure),String(temp2),String(light),String(soil));
  delay(20000);
}
